...
create or replace table lotstock
(
  vin varchar(25)
, exterior varchar(50)	
, interior varchar(50)
...
//create stage
create or replace STAGE aws_s3_bucket 
	URL = 's3://uni-cmcw/' 
...
//make sysadmin owner of the fileformat
grant ownership on file format util_db.public.CSV_COMMA_LF_HEADER to role sysadmin;

//back to sysadmin
use role sysadmin;

//now query the file with the format
select $1 as VIN
...
parse_header = TRUE;
//make sysadmin owner of the fileformat
grant ownership on file format util_db.public.CSV_COL_COUNT_DIFF to role sysadmin;
//back to sysadmin
use role sysadmin;

//Copy into the table where it finds a matched column name
...
//Doing something similar with local variables
...
//Final logic to update the table using the function
...
//Big ol chunk of code from the guide which goes through row by row and updates everything
